...
class RKB_OT_load(bpy.types.Operator):
    ...
    def execute(self, context):
        runtime_build = utility.load_build_config(
            Path(bpy.app.binary_path).resolve().parent.parent
        )
        ...
        bpy.context.window_manager.rkf.build = rocketfile.get(
            "build", bpy.context.window_manager.rkb.reference
        )
